...
class MysqlQueryBuilder implements SQLQueryBuilder
{
    ...
    /*
     * return $this в данном контексте возвращает текущий экземпляр
     * объекта MysqlQueryBuilder. Это позволяет использовать цепочку вызовов методов класса.
     * В данном случае, метод select должен вернуть экземпляр объекта
     * , реализующего интерфейс SQLQueryBuilder. Поскольку класс MysqlQueryBuilder
     * реализует этот интерфейс, он может вернуть самого себя ($this),
     *  что соответствует требованию интерфейса.
     * Использование return $this позволяет делать такие вещи:
     * $queryBuilder = new MysqlQueryBuilder();
     * $queryBuilder->select('users', ['id', 'name'])->where('id', 1)->limit(1)->build();
     */
